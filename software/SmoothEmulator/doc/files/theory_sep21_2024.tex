...
The goal is to find the coefficients, $A_{i>N_{\rm train}}$, that maximizes the probability given the constraints of matching the training data,
\begin{eqnarray}\label{eq:Nexact}
P(\vec{A})&\sim&\frac{1}{(2\pi\sigma_A^2)^{N_{\rm coef}/2}}\int \prod_{a=1}^{ N_{\rm train}}\delta[y(\vec{\theta}_a)-E(\vec{\theta}_a)]
\prod_{i=1}^{N_{\rm coef}}dA_i~\exp\left\{
...
\begin{align*}\eqnumber
\langle A_j\rangle&=\mathcal{N}^{-1}\int \prod_i \frac{dA_i}{\sqrt{2\pi\sigma_A^2}}~ A_j \exp\left[
-\sum_\ell\frac{A_\ell^2}{2\sigma_A^2}\right]\int \prod_a\frac{dk_a}{2\pi}\exp\left[ik_b(y_b-A_iT_i(\vec{\theta}_b))\right],\\
\langle A_jA_k\rangle&=\mathcal{N}^{-1}\int \prod_i \frac{dA_i}{\sqrt{2\pi\sigma_A^2}}~ A_jA_k \exp\left[
-\frac{A_i^2}{2\sigma_A^2}\right]\int \prod_a\frac{dk_a}{2\pi}\exp\left[ik_b(y_b-A_iT_i(\vec{\theta}_b))\right],\\
\mathcal{N}&=\int \prod_i \frac{dA_i}{\sqrt{2\pi\sigma_A^2}}~ \exp\left[
-\frac{A_i^2}{2\sigma_A^2}\right]\int \frac{dk}{2\pi}\exp\left[ik_b(y_b-A_\ell T_\ell(\vec{\theta}_b))\right].
\end{align*}
Here, $y_a=y(\vec{\theta}_a)$, the full model value calculated at the training point $a$.

After completing the square, the correlation can be written as
\begin{align*}\eqnumber
\langle A_jA_k\rangle &=\mathcal{N}^{-1}\int\prod_a\frac{dk_a}{2\pi}e^{ik_by_b}~\\
...
\right]\\
&=\mathcal{N}^{-1}
\int\prod_a\frac{dk_a}{2\pi}~\exp\left[-\frac{2}{2\sigma_A^2}(T_j(\vec{\theta}_b)k_b)^2+ik_by_b\right]\\
...
Similarly,
\begin{align*}\eqnumber
\langle A_j\rangle &=-\mathcal{N}^{-1}
\int\prod_a\frac{dk_a}{2\pi}~\exp\left[-\frac{1}{2\sigma_A^2}(T_j(\vec{\theta}_b)k_b)^2+ik_by_b\right]\\
...
\end{align*}
Doing the integrals for $\mathcal{N}$ and $\langle A_j\rangle$,
\begin{align*}\eqnumber\label{eq:netexactprob}
\mathcal{N}&=\sigma_A^{-N_{\rm train}}\left({\rm det}\mathcal{O}\right)^{-1/2}\exp\left[-\frac{1}{2\sigma_A^2}y_a\mathcal{O}^{-1}_{ab}y_b\right].
\end{align*}
One can then find the moments of the integral over $k$,
\begin{align*}\eqnumber
\langle k_a\rangle&=-i\partial_{y_a}\ln(\mathcal{N})=\frac{i}{\sigma_A^2}\mathcal{O}^{-1}_{ab}y_b,\\
\langle \delta k_a\delta k_b\rangle&=-\partial_{y_a}\partial_{y_b}\ln(\mathcal{N})=\frac{1}{\sigma_A^2}\mathcal{O}_{ab}^{-1}.
\end{align*}
The moments are then:
\begin{align*}\eqnumber\label{eq:exactmoments}
\langle A_j\rangle&=T_j(\vec{\theta}_a)\mathcal{O}^{-1}_{ab}y_b,\\
\langle\delta A_j\delta A_k\rangle&=\delta_{jk}\sigma_A^2-\sigma_A^2\mathcal{O}^{-1}_{ab}\mathcal{O}^{jk}_{ab}\\
&=\delta_{jk}\sigma_A^2-\sigma_A^2\mathcal{O}^{-1}_{ab}T_j(\vec{\theta}_a)T_k(\vec{\theta_b}).
\end{align*}

The observables and their fluctuations are then:
\begin{align*}\eqnumber
\langle y(\vec{\theta})\rangle&=\langle A_j\rangle T_j(\vec{\theta}),\\
\langle \delta y(\vec{\theta})^2\rangle&=
\langle \delta A_j\delta A_k\rangle T_j(\vec{\theta})T_k(\vec{\theta}).
\end{align*}
After definining
\begin{align*}\eqnumber
\chi_a&\equiv\mathcal{O}^{-1}_{ab}y_b,\\
S_a(\vec{\theta})&\equiv T_j(\vec{\theta})T_j(\vec{\theta}_a),
\end{align*}
the mean and fluctuation can be written as
\begin{align*}\eqnumber
\label{eq:exactsydely2}
\langle y(\vec{\theta})\rangle&=\chi_aS_a(\vec{\theta}),\\
\langle \delta y(\vec{\theta})^2\rangle&=
\sigma_A^2 T_j(\vec{\theta})T_j(\vec{\theta})
-\sigma_A^2S_a(\vec{\theta})\mathcal{O}^{-1}_{ab}S_b(\vec{\theta}).
\end{align*}
Whereas $\langle y(\vec{\theta})\rangle$ depends linearly on the training values, $y_{a=1,N_{\rm train}}$, the uncertainty, $\langle\delta y(\vec{\theta})^2\rangle$ depends only on the location of the training points. 

If one set $\vec{\theta}=\vec{\theta}_c$, one of the training points, then
\begin{align*}\eqnumber
S_a(\vec{\theta}_c)&=\mathcal{O}_{ac},\\
T_j(\vec{\theta}_c)T_j(\vec{\theta}_c)&=\mathcal{O}_{cc},~{\rm no~sum~over~}c,
\end{align*}
and
\begin{align*}\eqnumber
\langle \delta y(\vec{\theta}_c)^2\rangle
&=\sigma_A^2\mathcal{O}_{cc}-\sigma_A^2\mathcal{O}_{ca}
\mathcal{O}^{-1}_{ab}\mathcal{O}_{bc}=0.
\end{align*}
Thus, there is no uncertainty when evaluated at the training points.

Before considering a specific position, $\vec{\theta}$, one first calculates and stores the vector $\chi$ and the matrix $\mathcal{O}^{-1}$.  This involves inverting a $N_{\rm train}\times N_{\rm train}$ matrix. Then, when considering a specific $\vec{\theta}$, one calculates $S_a(\vec{\theta})$ for each $a$. This involves calculating a double sum with $N_{\rm train}\times N_{\rm coef}$ elements. Once these are stored, calculating $\langle\delta y^2\rangle$ involves calculating a double sum with an additional $N_{\rm train}\times N_{\rm train}$ elements. Thus, calculating $S_a(\vec{\theta})$ is where the bulk of the calculation occurs.  Because $N_{\rm train}\lesssim 100$ and $N_{\rm coef}$ might be of in the tens of thousands (depending on the number of parameters and maximum rank), one should be able to calculate $\langle y\rangle$ and $\langle\delta y^2\rangle$ in much less than a second. Nonetheless, hen performing an MCMC calculation, one might calculate for millions of values of $\vec{\theta}$, which can push the calculation into the ranges of minutes or even hours if one has a dozen parameters or more.
...
\begin{align*}\eqnumber\label{eq:netexactprob}
\int \prod_i dA_i P(\vec{A},\sigma^2)&=\mathcal{N}\\
&=\sigma_A^{-N_{\rm train}}\left({\rm det}\mathcal{O}\right)^{1/2}\exp\left[-\frac{1}{2\sigma_A^2}y_a\mathcal{O}^{-1}_{ab}y_b\right].
\end{align*}
Our goal here is to find the value of $\sigma_A$ that maximizes $\mathcal{N}$. Setting the derivative w.r.t. $\sigma_A^2$ equal to zero,
\begin{align*}\eqnumber
0&=\frac{N_{\rm train}}{\sigma_A^2}\mathcal{N}-\frac{1}{\sigma_A^4}y_a\mathcal{O}^{-1}_{ab}y_b,\\
\sigma_A^2&=\frac{1}{N_{\rm train}}y_a\mathcal{O}^{-1}_{ab}y_b.
\end{align*}
