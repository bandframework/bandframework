...
\subsection{Overview}
The {\it Smooth Emulator} distribution includes three plotting programs, all based on Pythons's Matplotlib. The first program allows the User to visualize the accuracy of the emulator by comparing emulator predictions to full-model calculations at random points not used to train the emulator. The other two plotting programs illustrate the results of the MCMC constraint of model-parameter space.

Before running any of the Python scripts below, the User may wish to use {\tt pyenv} so that the desired versions of Python are called. The User might want to run {\tt python3 -- version} to ensure that the desired version of Python is being installed. If running on a Mac using {\it homebrew}, that would be {\tt /opt/homebrew/bin/python3}. For some, it might be simpler to replace the {\tt python3} command with one specifying the entire path.

\subsection{Preparing the Data Files for the Plots}
The plotting programs are contained the {\tt  \$\{MY\_ANALYSIS\}/figs/} directory. The three plotting programs are located in the three subdirectories, {\tt figs/YvsY}, {\tt figs/posterior} and {\tt figs/resolvingpower}. Before running the plotting programs, one must first create several data files and move them to the {\tt figs/figdata} directory. This directory will hold all data required to make the plots. Thus, none of the plotting functionality depends on the location of the {\tt figs/} directory, as long as the {\tt figdata/} directory is a subdirectory of the same directory as {\tt YvsY/}, {\tt posterior/} and {\tt resolvingpower/}.
...
\vspace*{-8pt}{\tt
\begin{Verbatim}
% cp \textit{MY_ANALYSIS}/smooth_data/MCMC/trace_theta.txt \textit{MY_ANALYSIS}/figs/figdata/
% cp \textit{MY_ANALYSIS}/smooth_data/MCMC/ResolvingPower.txt \textit{MY_ANALYSIS}/figs/figdata/
\end{Verbatim}
}\vspace*{-8pt}
...
\begin{figure}[h]
\centerline{\includegraphics[width=0.8\textwidth]{figs/posterior_tutorial}}
