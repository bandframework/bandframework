...
\vspace*{-8pt}{\tt
\begin{Verbatim}[commandchars=\\\{\}]
..% \textcolor{darkred}{cd $\{SMOOTH_HOME\}/software/pybind_stuff}
$\{SMOOTH_HOME\}/software/pybind_stuff% \textcolor{darkred}{cmake .}
$\{SMOOTH_HOME\}/software/pybind_stuff% \textcolor{darkred}{make}
\end{Verbatim}
}\vspace*{-8pt}

\subsection{Editing and Running a Python Script}
The Python script must be located in a analysis directory, i.e. one where there is a {\tt smooth\_data/} directory set up in the manner required by the emulator software. An example Python script is in the {\tt \$\{SMOOTH\_HOME\}/software/pybind\_stuff/} directory. It can be moved to the analysis directory.
\vspace*{-8pt}{\tt
\begin{Verbatim}[commandchars=\\\{\}]
..% \textcolor{darkred}{cp $\{SMOOTH_HOME\}/software/pybind_stuff/smoothy_emulate.py $\{MY_ANALYSIS\}/}
..% \textcolor{darkred}{cd $\{MY_ANALYSIS\}/} 
\end{Verbatim}
}\vspace*{-8pt}
If the User has defined an environment variable, {\tt \$\{SMOOTH\_HOME\}}, to the parent directory of {\tt software/pybind\_stuff}, the script is ready to run. Otherwise the User needs to edit the following line from the Python script.
\vspace*{-8pt}{\tt
\begin{Verbatim}[commandchars=\\\{\}]
...
The easiest way to add more functions is to edit and extend the file {\tt \$\{SMOOTH\_HOME\}/software/pybind\_stuff/pybind\_main.cc}. One can see how all the functions above were defined. One can extend this to more of the functions defined in the header file {\tt \$\{SMOOTH\_HOME\}/software/include/msu\_smooth/master.h}. However, one should remember that the binding cannot easily change the functionality of functions with arguments passed by reference. Thus, if the User wishes to find some variable, e.g. {\tt Z}, from some C++ function that is written in the form {\tt void ThisIsMyMethodToGetZ(double \&Z,double x,double y)}, they will instead have to write a new C++ function in the form {\tt double ThisIsMyMethodToGetZ(double x,double z)}, where the new function returns {\tt Z} rather than having it updated by the function. For example to return the emulated value and uncertainty, a function was created of the form {\tt vector<double> GetYSigmaFromXPython(int iY,vector<double> X)}. The returned vector was 2-dimensional with {\tt Y[iY]} being the first element and {\tt sigmaY[iY]} being the second element.
