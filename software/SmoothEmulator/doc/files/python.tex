...
\vspace*{-8pt}{\tt
\begin{Verbatim}[commandchars=\\\{\}]
..% \textcolor{darkred}{cd \textit{SMOOTH_HOME}/software/pybind_stuff}
\textit{SMOOTH_HOME}/software/pybind_stuff% \textcolor{darkred}{cmake .}
\textit{SMOOTH_HOME}/software/pybind_stuff% \textcolor{darkred}{make}
\end{Verbatim}
}\vspace*{-8pt}

\subsection{Editing and Running a Python Script}
The Python script must be located in a analysis directory, i.e. one where there is a {\tt smooth\_data/} directory set up in the manner required by the emulator software. An example Python script is in the \textit{SMOOTH\_HOME}{\tt /software/pybind\_stuff/} directory. It can be moved to the analysis directory.
\vspace*{-8pt}{\tt
\begin{Verbatim}[commandchars=\\\{\}]
..% \textcolor{darkred}{cp \textit{SMOOTH_HOME}/software/pybind_stuff/smoothy_emulate.py \textit{MY_ANALYSIS}/}
..% \textcolor{darkred}{cd \textit{MY_ANALYSIS}/} 
\end{Verbatim}
}\vspace*{-8pt}
If the User has defined an environment variable, \textit{SMOOTH\_HOME}{\tt }, to the parent directory of {\tt software/pybind\_stuff}, the script is ready to run. Otherwise the User needs to edit the following line from the Python script.
\vspace*{-8pt}{\tt
\begin{Verbatim}[commandchars=\\\{\}]
...
\vspace*{-8pt}{\tt
\begin{Verbatim}[commandchars=\\\{\}]
\textit{MY_ANALYSIS}% \textcolor{darkred}{python3 \textit{SMOOTH_HOME}/software/pybind_stuff/smoothy_emulate.py}
