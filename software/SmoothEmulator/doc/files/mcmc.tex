...
\subsection{Overview}
The {\tt \$\{SMOOTH\_HOME\}/software/MainPrograms/} directory contains the source code for the main program, {\tt smoothy\_mcmc\_main.cc}. When compiled it creates an executable, {\tt \$\{SMOOTH\_HOME\}/software/bin/smoothy\_mcmc}, which then performs an MCMC analysis. This program will invoke the emulator in its exploration of the model-parameter space. It will write the trace to file and perform and record some analysis of the trace the User might find useful. Even if the User chooses a different MCMC approach then the one provided with the {\it Smooth Emulator} software, viewing the source code might prove useful in demonstrating how to invoke the emulator from within a C++ code. 
...
\textit{MY_ANALYSIS}% \textcolor{darkred}{$\{SMOOTH_HOME\}/bin/smoothy_mcmc}
...
Finally, we review the source code in {\tt \$\{SMOOTH\_HOME\}/software/MainPrograms/mcmc\_main.cc}:
{\tt
\begin{Verbatim}[commandchars=\\\{\}]
int main()\{
