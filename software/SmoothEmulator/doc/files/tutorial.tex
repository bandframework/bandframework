...
Here, {\tt ../SmoothHome}, {\tt ../MyAnalysis} and {\tt ../MyLocal} can be replaced by any directory name you choose. For the rest of the tutorial the following shorthand will be applied\\
...
{\tt \$\{SMOOTH\_LOCAL\}} & \parbox{5in}{Directory tree copied from {\tt \$\{SMOOTH\_HOME\}/LocalTemplate}. Executables are store in {\tt \$\{SMOOTH\_LOCAL\}/bin} and main programs, and source codes for main\\ programs are found within {\tt \$\{SMOOTH\_LOCAL\}/software/main\_programs}\\}\\
...
You should be sure to have a C++ compiler that handles the C++-20 standard. You should also have Cmake and the Eigen linear algebra packages installed. You should also need a Python distribution installed along with the matplotlib package. You can now compile the main libraries
\vspace*{-8pt}{\tt \begin{Verbatim}[commandchars=\\\{\}]
...
{\tt \begin{Verbatim}[commandchars=\\\{\}]
   ...
   % \textcolor{darkred}{export SMOOTH_LOCAL=$\{SMOOTH_LOCAL\}}
   \end{Verbatim}
}\vspace*{-8pt}
Here, {\tt \$\{SMOOTH\_HOME\}} and {\tt \$\{SMOOTH\_LOCAL\}} are the full paths to the locations chosen above. You might find it convenient to add {\tt \$\{SMOOTH\_LOCAL\}/bin} to your path.
\vspace*{-8pt}
{\tt \begin{Verbatim}[commandchars=\\\{\}]
   ..% \textcolor{darkred}{export PATH=$\{PATH\}:$\{SMOOTH_LOCAL\}/bin }
   \end{Verbatim}
}\vspace*{-8pt}
...
Your output might vary a bit because the random seed for the MCMC search is initialized by the current time. If you have defined your path to include {\tt \$\{SMOOTH\_LOCAL\}/bin/}, you can invoke the command by simply entering {\tt trainingpoint\_optimizer}.
