...
void CalcY(vector<double> &xmin,vector<double> &xmax,vector<double> &x,vector<double> &Y,Crandy *randy){
	...
	double Lambda0=NPars+3,rg;
	for(unsigned int I=0;I<NPars;I++){
		
		for(unsigned int i=0;i<NPars;i++){
			for(unsigned int j=0;j<NPars;j++){
				rg=randy->ran_gauss();
				t[I]+=0.5*rg*theta[i]*theta[j]/Lambda0;
				
				for(unsigned int k=0;k<NPars;k++){
					rg=randy->ran_gauss();
					t[I]+=rg*theta[i]*theta[j]*theta[k]/(6.0*Lambda0*Lambda0);
					
					for(unsigned int ell=0;ell<NPars;ell++){
						rg=randy->ran_gauss();
						t[I]+=rg*theta[i]*theta[j]*theta[k]*theta[ell]/(24.0*Lambda0*Lambda0*Lambda0);
						//nterms+=1;
					}
	...
	Y[1]=725.0+150.0*cos(t[1]/Lambda);
	Y[2]=1100.0+200.0*(t[2]+exp(t[2]/Lambda));
	Y[3]=5.5+2.5*t[3];
	Y[4]=0.4+0.2*t[4]+0.25*log(1.0+(t[4]*t[4]/((1.0+t[4]*t[4]))/(Lambda*Lambda)));
	Y[5]=0.6+0.4*t[5];
}
