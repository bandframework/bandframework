...
NObs=0
iline=0
for line in open('../figdata/observable_info.txt'):
  if len(line) > 1:
    NObs+=1
...
NPars=0
iline=0
for line in open ('../figdata/prior_info.txt'):
  if len(line) > 2:
    tempdata=list(map(str,line.split()))
    NPars=NPars+1
...
RPdata = np.loadtxt("../figdata/ResolvingPower.txt",skiprows=0,unpack=False)
