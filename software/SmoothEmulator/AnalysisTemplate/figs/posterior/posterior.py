    ...
    if ipanel==jpanel:
      ...
      for i in range(0,npts):
        ...
        if ibin >= nbins or ibin < 0:
            if priordata[ipar][1] == 'uniform' and fabs(x) > 1.001:
               print ('Outside (-1,1) interval (uniform dist), Theta = ', x)
            if priordata[ipar][1] == 'gaussian' and fabs(x) > 1.5:
               print ('Rather outside (-1.5,1.5) interval (gaussian dist), Theta = ', x)
        else:
